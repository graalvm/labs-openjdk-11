...
C2V_VMENTRY_0(jlong, getExceptionTableLength, (JNIEnv* env, jobject, ARGUMENT_PAIR(method)))
...
C2V_VMENTRY_NULL(jobject, findUniqueConcreteMethod, (JNIEnv* env, jobject, ARGUMENT_PAIR(klass), ARGUMENT_PAIR(method)))
  ...
  if (method->can_be_statically_bound() || method->is_private()) {
    JVMCI_THROW_MSG_NULL(InternalError, err_msg("Effectively static method %s.%s should be handled in Java code", method->method_holder()->external_name(), method->name_and_sig_as_C_string()));
  }
...
C2V_VMENTRY_0(jint, installCode0, (JNIEnv *env, jobject,
  ...
  objArrayOop object_pool_oop;
  if (JVMCIENV->is_hotspot()) {
    object_pool_oop = (objArrayOop) JNIHandles::resolve(object_pool);
  } else {
    object_pool_oop = NULL;
  }
  objArrayHandle object_pool_handle(thread, object_pool_oop);
  CodeBlob* cb = NULL;
...
C2V_VMENTRY_NULL(jobject, getHostClass, (JNIEnv* env, jobject, ARGUMENT_PAIR(klass)))
  InstanceKlass* k = InstanceKlass::cast(UNPACK_PAIR(Klass, klass));
...
C2V_VMENTRY_NULL(jobject, readFieldValue, (JNIEnv* env, jobject, jobject object, ARGUMENT_PAIR(expected_type), long displacement, jchar type_char))
  ...
  bool is_static = expected_klass == NULL && java_lang_Class::is_instance(obj()) && displacement >= InstanceMirrorKlass::offset_of_static_fields();
...
JNINativeMethod CompilerToVM::methods[] = {
  ...
  {CC "getHostClass",                                 CC "(" HS_KLASS2 ")" HS_KLASS,                                                        FN_PTR(getHostClass)},
